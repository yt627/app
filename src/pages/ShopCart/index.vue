      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="cart in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con7">
            <a href="#none" class="sindelet" @click="deleteCartShopById(cart)">删除</a>
            <br>
            <a href="#none">移到收藏</a>
          </li>
        </ul>
  ...
  export default {
    ...
    methods:{
      ...
      handler:throttle(async function(type, disNum, cart){
        // type:为了区分这三个元素  + - input
        // disNum：+变化量（+1）   -变化量（-1）   input最终的个数并不是变化量
        // cart:哪一产品（身上有id）
        // 向服务器发请求，修改数量
        switch (type) {
          case 'add':
            ...
            break;
          case 'minus':
            // 有bug，快速点击会出现负数，    应该在用户输入时判断合不合法，使用正则？
