<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>确认密码:</label>
        <input placeholder="请确认密码" v-model="password1" name="password" v-validate="{ required: true,is:password}" :class="{ invalid: errors.has('password1') }" />
  ...
  export default {
    ...
    methods:{
        ...
        async userRegister(){
            ...
            // 全部表单验证，再向服务器发请求，进行注册
            // 只要有一个没成功，就不通过
            if (success) {
                try {
                    const { phone, code, password, password1 } = this;
                    (phone && code && password && password == password1) && (await this.$store.dispatch('userRegister', { phone, code, password, password1 }));
